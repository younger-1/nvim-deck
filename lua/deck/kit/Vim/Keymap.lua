...
function Keymap.send(keys, no_insert)
  ...
  return Async.new(function(resolve, _)
    ...
    local callback = Keymap.termcodes(('<Cmd>lua require("deck.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
    if no_insert then
...
function Keymap.to_sendable(callback)
  ...
  return Keymap.termcodes(('<Cmd>lua require("deck.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
end
